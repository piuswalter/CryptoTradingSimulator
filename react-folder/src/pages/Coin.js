...
export default class Coin extends React.Component {
    ...
    setTextAndLogo() {
        ...
        switch (this.state.symbol) {
            case 'btc': text = `Bitcoin is the world's leading cryptocurrency based on a decentrally organized system.
                                Payments are cryptographically authenticated and processed via a network of computers.`;
                logo = BTC_logo; break;
            case 'eth': text = `Ethereum is the second largest cryptocurrency platform by market capitalization, behind Bitcoin. 
                                It is a decentralized open source blockchain featuring smart contract functionality.`;
                logo = ETH_logo; break;
            case 'usdt': text = `Tether is an unregulated crypto currency with tokens issued by Tether Limited. 
                                Tether is traded as a so-called stablecoin 1:1 to the US dollar.`;
                logo = USDT_logo; break;
            case 'xrp': text = `XRP is an open source protocol for a payment network, based on an idea by web developer Ryan Fugger, 
                                businessman Chris Larsen and programmer Jed McCaleb. It is further developed by Ripple Labs.`;
                logo = XRP_logo; break;
            case 'bch': text = `Bitcoin Cash is a crypto currency that was created on August 1, 2017 through a hard fork from 
                                the Bitcoin network. Bitcoin Cash is the fourth largest crypto currency by market capitalization.`;
                logo = BCH_logo; break;
            case 'bsv': text = `Bitcoin SV is a crypto currency based on the Bitcoin protocol. Its goal is to restore the original 
                                protocol rules and "freeze" these rules, as intended by Bitcoin inventor Satoshi Nakamoto.`;
                logo = BSV_logo; break;
            case 'ltc': text = `Litecoin is a peer-to-peer cryptocurrency and open-source software project. Transfer of coins is 
                                based on an open source cryptographic protocol and is not managed by any central authority`;
                logo = LTC_logo; break;
            case 'bnb': text = `The Chinese Exchange Binance introduced the native token Binance Coin on 8 July 2017. It is a token 
                                exclusively for their own trading exchange.`;
                logo = BNB_logo; break;
            case 'eos': text = `EOS.IO is a blockchain protocol powered by the native cryptocurrency EOS. The smart contract 
        ...
        return (
            <Container fluid>
                    ...
                    <Col md='4' className='h-100 m-0 p-0 pr-2 pb-2 pl-2'>
                        ...
                        <div className='h-14_5 pt-2'>
                            <div className='rounded text-center' style={{ backgroundColor: '#131821', height: '100%', border: '2px solid grey' }}>
                                ...
                                <Table striped bordered hover variant="dark" className="w-100">
                                    <tr><td>{currentCoin}x</td><td> ${(price * currentCoin).toFixed(2)}</td></tr>
                                </Table>
                            </div>
                        </div>
                        <div className='h-28 pt-2'>
                            <div className='rounded text-center' style={{ backgroundColor: '#131821', height: '100%', border: '2px solid grey' }}>
                                ...
                                <Table striped bordered hover variant="dark" className="w-100">
                                    <tr><td>Change hour</td><td id='changeH'>{changeH} %</td></tr>
                                    <tr><td>Change day</td><td id='changeD'>{changeD} %</td></tr>
                                    <tr><td>Change week</td><td id='changeW'>{changeW} %</td></tr>
                                </Table>
                            </div>
                        </div>
                                ...
                                <Form className='w-100 h-100'>
                                    ...
                                    <div className='h-30 d-flex justify-content-center'>
                                        <div className='w-50 h-100 d-flex justify-content-center'>
                                            <Button className='w-95 h-90 bg-green border-0' onClick={this.handleBuy} disabled={this.state.loading}><h4 className='m-auto'>Buy</h4></Button>
                                        </div>
                                        <div className='w-50 h-100 d-flex justify-content-center'>
                                            <Button className='w-95 h-90 bg-red border-0' onClick={this.handleSell} disabled={this.state.loading}><h4 className='m-auto'>Sell</h4></Button>
                                        </div>
                                    </div>
                                </Form>
                            </div>
                ...
                <Modal size="sm" centered show={this.state.buySuccess} onHide={() => this.setState({ buySuccess: false, loading: false })}>
                    ...
                    <Modal.Body className='bg-dark d-flex justify-content-center align-items-center'>
                        <Table striped bordered hover variant="dark">
                            <tr><td>Bought:</td><td>{coinsBought + " " + symbol.toUpperCase()}</td></tr>
                            <tr><td>Total:</td><td>{currentCoin + " " + symbol.toUpperCase()}</td></tr>
                ...
                <Modal size="sm" centered show={this.state.sellSuccess} onHide={() => this.setState({ sellSuccess: false, loading: false })}>
                    ...
                    <Modal.Body className='bg-dark d-flex justify-content-center align-items-center'>
                        <Table striped bordered hover variant="dark">
                            <tr><td>Sold:</td><td>{coinsSold + " " + symbol.toUpperCase()}</td></tr>
                            <tr><td>Total:</td><td>{currentCoin + " " + symbol.toUpperCase()}</td></tr>
                            <tr><td>Balance:</td><td>${currentUSD}</td></tr>
                        </Table>
                    </Modal.Body>
